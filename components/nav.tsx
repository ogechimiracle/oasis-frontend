...
function Nav() {
  ...
  return (
    <nav
      className={`relative h-15 z-40 ${path === "/" ? "bg-myheroColor" : "bg-white"} `}
    >
      <div
        className={`absolute inset-0 py-4  w-full ${path === "/" ? "bg-black/15" : ""}`}
      >
        <div className="lg:px-15 px-5 flex items-center justify-between">
          <div className="flex items-center gap-x-4">
            ...
            />
            <span
              className={`text-lg lg:text-lg font-semibold ${path === "/" ? "text-white" : "text-black"}`}
            >
              Integrated Oasis
            </span>
          </div>
          ...
          <div className="hidden lg:block">
            <ul className={`flex items-center gap-x-10 ${path ==="/"? "text-white":"text-black"}`}>
                    ...
                    <li key={index} className="relative group">
                      ...
                      >
                        <div className="p-2">
                          <Link
                            href="/courses"
                            className="block px-4 py-3 rounded-lg text-sm  hover:bg-myprimaryColor text-black"
                          >
                            All Courses
                          </Link>
                          {categoriesLoading ? (
          ...
          <div className="hidden lg:block">
            {user ? (
              <div className="cursor-pointer relative group">
                ...
                >
                  <div className="flex flex-col">
                    <Link
                      href=""
                      className="py-3 px-4 hover:bg-myprimaryColor text-black"
                    >
                      Dashboard
                    </Link>
                    <Link
                    ...
                    <button
                      ...
                      className="text-left px-4 py-2.5 cursor-pointer hover:bg-myprimaryColor text-black inline-flex items-center gap-x-3"
                    >
                      <span>Sign Out</span>
                      <MdOutlineLogout />
                    </button>
              ...
              <div className="flex items-center gap-x-4">
                <Link
                  href="/auth/"
                  className="px-6 py-2 rounded-full text-sm bg-white shadow-sm text-black cursor-pointer"
                >
                  Login
                </Link>
                <Link
                  href="/auth/"
                  className="px-6 py-2 rounded-full text-sm bg-myprimaryColor shadow-sm text-black cursor-pointer font-poppins "
                >
                  Sign Up
                </Link>
              </div>
          ...
          >
            <div className="px-8 py-5 mt-5">
              <ul className="flex flex-col items-center gap-y-4">
                {user && <Link href="">Dashboard</Link>}

                {oasisNavLinks.map((link, index) => {
                  // Courses gets special treatment
                  if (link.name === "Courses") {
                    return (
                      <li
                        key={index}
              ...
              {!user ? (
                ...
                <button
                  onClick={handleLogout}
                  className=" px-4 py-2.5 cursor-pointer hover:bg-myprimaryColor text-black bg-myprimaryColor w-full inline-flex items-center justify-center gap-x-3 mt-4"
                >
                  <span>Sign Out</span>
                  <MdOutlineLogout />
                </button>
